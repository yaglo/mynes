 ...
 * resolved):
 *   <config-dir>/saves/<name>.sav    the 8 KB PRG RAM window ($6000-$7FFF)
...
#define MYNES_PRG_RAM_SIZE   0x2000
...
typedef struct {
    ...
    uint8_t  on_disk[MYNES_PRG_RAM_SIZE];   /* PRG RAM as last read from or written to disk */
} MynesSaves;
...
/* Bind `s` to a cartridge. Nothing is read from disk yet. */
void mynes_saves_open(MynesSaves *s, const char *rom_path, uint32_t rom_crc, bool has_battery);

/* Copy <saves>/<name>.sav into prg_ram. Returns true when a file was read; a
 * missing file leaves prg_ram alone (a fresh cartridge) and returns false. */
bool mynes_saves_restore(MynesSaves *s, uint8_t *prg_ram);

/* Write prg_ram to <saves>/<name>.sav when it differs from the copy on disk,
