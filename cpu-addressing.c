...
#if OP_TRACE
    printf("%02X    ", memory_readb(cpu.PC - 1));
#endif
}
...
#if OP_TRACE
    printf("%02X %02X ", op_address & 0xFF, op_address >> 8);
#endif
}
...
#if OP_TRACE
    printf("%02X %02X ", memory_readb(cpu.PC - 2), memory_readb(cpu.PC - 1));
#endif
}
