...
{
    // printf("READING %04X = %04X\n", address, ppu_get_real_ram_address(address));
...
inline byte ppu_io_read(word address)
{
    ...
    //         return value;
    //     }
    //     case 0x2004: return PPU_SPRRAM[ppu.OAMADDR];
    //     case 0x2007:
    //     {
    //         byte data = PPU_RAM[ppu.PPUADDR];
    //         ppu.PPUADDR = (ppu.PPUADDR + ppu_vram_address_increment()) & 0x3FFF;
    //         return data;
    //     }
    //     default:
    //         return 0xFF;
...
inline void ppu_io_write(word address, byte data)
{
    ...
    switch(address) {
        ...
        case 7:
        {
            ...
            // if (ppu.PPUADDR <= 0x1FFF) {
            //     PPU_RAM[ppu.PPUADDR] = data;
            // }
            // else if (ppu.PPUADDR <= 0x3EFF) {
            //     PPU_RAM[ppu.PPUADDR ^ ppu.mirroring_xor] = data;
            //     PPU_RAM[ppu.PPUADDR] = data;
            // }
            // else if (ppu.PPUADDR >= 0x3F00) {
            //     if (ppu.PPUADDR == 0x3F00) {
            //         PPU_RAM[0x3F00] = PPU_RAM[0x3F04] = PPU_RAM[0x3F08] = PPU_RAM[0x3F0C] = data;
            //     }
            //     else if (ppu.PPUADDR == 0x3F10) {
            //         PPU_RAM[0x3F14] = PPU_RAM[0x3F18] = PPU_RAM[0x3F1C] = data;
            //     }
            //     else {
            //         PPU_RAM[ppu.PPUADDR] = data;
            //     }
