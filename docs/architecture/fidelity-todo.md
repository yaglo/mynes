...
- [~] 1. H/H/S. Draw NES pixels at 8:7 and place the picture on the receiver's active raster. Done: the deflection map now uses BT.470 blanking and the console's timings, so dots are 8:7; the decoder covers the whole active raster (`decode_window.h`), so the 15+11 border dots of backdrop show at the sides, with flyback blanking outside them; household presets take the documented alignment (JVC 5% H and 4% V per edge, Toshiba V 9±2% as 4.5% per edge, the JVC's figure for the sets without one), so the border hides as on the set. Open: the PVM's underscan raster is 252×188 mm, smaller than the face, with about 13/10 mm backdrop borders; the other named sets' own alignment figures. WP-F.
...
- [x] 12. M/H/S. One physical noise origin on RF. Done: `rf.modulator_dbmv` (the FCC 15.115 cap of 9.5 dBmV by default), `rf.link_loss_db` and `rf.tuner_nf_db` give the carrier and the tuner's kTF noise (`video_rf_link_budget`), with the CNR in 4 MHz as a read-only line in the RF menu; old presets' carrier and noise pairs become the budget that gives their ratio; the RF presets keep their former CNRs as losses (Basement 21.4 dB, Bedroom 19.4, Stas 11.4, Zenith 11.4, Famicom 7.4 with its modulator 6 dB low) and lose their gun grain and lead pickup; the FM sound's hiss now takes the sound carrier's CNR in the 200 kHz sound IF, about 23 dB less hiss than before; the gun noise seed is mixed per frame, line and position (frame f+2 was frame f shifted 85 lines). `test_fidelity` holds the detected snow to the analytic level within 5% for both detectors. Open: the NES modulator's real level (measurement B3) and each tuner's noise figure. WP-B, WP-E, WP-P.
