...
- [~] 5. H/H/S. RF sound. Done: 75 µs (50 µs PAL) de-emphasis on RF presets (`audio_rf_deemphasis`, auto), FM hiss from the RF stage's carrier and noise levels with the sound carrier assumed 13 dB under vision. Open: the NES modulator's actual sound level and deviation (measurement B3). WP-I.
...
- [~] 28. M/M-L/M. TV audio circuits. Done: PVM-14L2 (AN5278, 100 µF into the 7×5 cm speaker, 199 Hz) and Toshiba 14AF43 (AN5276, 1000 µF into 8 Ω, 20 Hz) output networks as speaker profiles; the AV follower simulated in the audio deck (0.05–0.21% THD at NES levels, so linear); ripple-derived hum; mains pickup as a setting. Open: the drivers are class estimates; the field-rate buzz; CPU audio chain by default. WP-I.
...
- [ ] 30. L/M/M. Console PSU fault model (7805 dropout scales the DAC ratiometrically), fault presets only. WP-E.
...
**WP-B, RF link and front end.** `[ ]` Detector type per receiver (item 2); keyed top-sync AGC across frames (item 4); link budget and CNR (item 12); multipath at complex IF (item 23). Reference decks `nes001_rf_modulator` and `famicom_rf_modulator` (reference only until an SDR capture exists). Test `test_rf_front.c`.
