...
Horizontal spot growth uses two normalized kernels weighted by each source gun's current, so widening deposits light rather than inventing it. The PVM preset's vertical FWHM runs from 0.38 line at low drive to 0.60 at full white, with integrated-energy error below 0.01%. The white value comes from the maintainer's PVM, on which full-white scanlines stay separated: at the earlier 0.90, adjacent white lines filled the gap between them to 82% of their peak in linear light, and at 0.60 the gap is 33%. It is an observation, not a measured spot profile.
