...
| Audio | Processing backend, volume, Analog character: gate rail window, cable pickup, TV input resistance, RF sound de-emphasis, PSU ripple amplitude/frequency/harmonics, extra noise, console audio circuit, speaker model (incl. PVM-14L2 and Toshiba 14AF43 networks), audio cable | The console filter, rail window and ripple come from the NES-001 schematic through ngspice; the PVM and Toshiba speaker capacitors from their service manuals. Hum runs at mains when pickup is set and at twice mains for the ripple alone. |
