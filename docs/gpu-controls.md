...
| Console | Output resistance, video bandwidth, differential-phase RC, output follower RC, PSU hum | Nonlinear phase response and the follower apply to NTSC composite/RF. Output resistance also affects the cable pole. The follower RC is the NES-001 emitter follower's rise limit (R2 510 Ω charging C5 330 pF, 168 ns) from the schematic, simulated in `tools/circuits/nes001_video_chain.cir`; steps down are followed at once, steps up towards +5 V, so bright detail rises slowly and each palette row gets its own chroma gain and phase. 0 is off. |
...
| RF | Modulator level (dBmV), link loss, tuner noise figure and the resulting CNR in 4 MHz (read-only), complex IF bandwidth/asymmetry/detuning, AGC attack tau and 52 dB release time, video detector, sound AM rejection, modulator ICPM | Applies to the RF route. Carrier *frequency* is legacy channel metadata, not a simulated tunable RF oscillator. The AGC is the set's keyed top-sync loop: one capacitor charged by the sync tip's excess over the reference and discharged at a constant rate (TDA8362: 2 ms and 52 dB per 25 ms), carried across lines and frames, so snow no longer paints line-to-line gain streaks. The video detector is synchronous (a PLL vision IF, every one-chip IF since the late 1970s) unless set to envelope for a pre-1978 diode detector, which carries the Nyquist slope's quadrature into the picture as a level shift on chroma and edges. The two sound items are what the buzz is derived from: each frame's picture sets the vision carrier's envelope, the set's intercarrier detector passes that envelope's field and line structure at its AM rejection (45 dB typical of a quadrature detector), and the console modulator's incidental phase modulation adds a tick where the level steps at blanking. Carrier level and noise floor also set the FM hiss. |
...
| Audio | Processing backend, volume, Analog character: console supply (adaptor VAC, reservoir µF, console load mA, 7805 rejection dB), cable pickup, TV input resistance, RF sound de-emphasis, extra noise (diagnostic), console audio circuit, speaker model (the generic classes and the named sets: PVM-14L2, Toshiba 14AF43, JVC AV-27D201, Sony KV-27FS120, Commodore 1702, Zenith 19-inch, RCA ColorTrak, PVM-20M4U, NEC XM29), audio cable | Nothing about hum is set directly: the supply block is run as its circuit (bridge, reservoir, regulator with dropout) and the ripple it leaves on +5 V reaches the jack through the NES-001 gate's supply gain, at twice mains with the circuit's harmonics; lower the adaptor voltage or the reservoir and the hum appears by itself. The gate's rail window is the chip's, VCC − 2 V. Pickup runs at mains through what the cable's shield lets past. The PVM and Toshiba speaker capacitors come from their service manuals. |
