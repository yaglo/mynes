...
For the original cadence and longer sequence, use the **120-frame, 60.0988 fps MP4s**:

| Profile | Video | Frame-by-frame details |
|---|---|---|
| Sony PVM-14L2 | [Play](images/motion/boss-sony_pvm_14l2.mp4) | [Sequence](images/motion/boss-sony_pvm_14l2-sequence.png) |
| JVC D-Series | [Play](images/motion/boss-jvc_d_series_2000.mp4) | [Sequence](images/motion/boss-jvc_d_series_2000-sequence.png) |
| Toshiba 14AF | [Play](images/motion/boss-toshiba_14af43.mp4) | [Sequence](images/motion/boss-toshiba_14af43-sequence.png) |
| Stas's Favourite | [Play](images/motion/boss-stass_favourite.mp4) | [Sequence](images/motion/boss-stass_favourite-sequence.png) |
...
The frozen boss provides a useful separation of signal motion from game motion. Across its 120 frames, median adjacent-frame linear RGB RMS was 0.0491 for Sony, 0.0398 for JVC, 0.0289 for Toshiba and 0.0405 for Stas. Yet the largest channel's peak-to-peak whole-image mean changed by less than 0.0008 in every profile. The change is principally local, rather than a large global exposure pulse. This does **not** measure perceived flicker on a particular panel.
...
The inspected sequence shows terrain moving across a fixed output mask. Sony keeps the harder edges; Stas spreads moving detail more broadly. Neither sampled sequence shows a long retained copy of the terrain, although this short review is not a measured phosphor-decay calibration. [Sony frame sequence](images/motion/contra-gameplay-sony_pvm_14l2-sequence.png) · [Stas frame sequence](images/motion/contra-gameplay-stass_favourite-sequence.png).

[Sequence measurements](gpu-motion-metrics.json) include all source frame numbers and per-frame light statistics. Every exported clip contains 120 consecutive source frames.

## Capture method

All clips use the actual GPU path, Panel-pixels mask alignment, a 960×720 offscreen drawable and common 0.7 linear exposure before sRGB encoding. The boss input is frozen, while the carrier phase, noise and temporal state continue advancing. Capture logs verify every frame from 60 through 179 without gaps. Screenshot readback may run slower than real time; the exported video uses emulated-frame cadence, not disk-write timestamps. No optical-flow interpolation or frame averaging is applied.
