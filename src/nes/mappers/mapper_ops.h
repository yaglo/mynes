...
typedef struct MapperOps {
    ...
    /* Debugger reads: pure lookups that never write the mapper, so another
     * thread can call them while the console runs. Only mappers whose
     * cpu_read/ppu_read change state provide them; the others are peeked
     * through their plain read. */
    uint8_t (*cpu_peek)(const Mapper *m, uint16_t addr);
    uint8_t (*ppu_peek)(const Mapper *m, uint16_t addr);
} MapperOps;
