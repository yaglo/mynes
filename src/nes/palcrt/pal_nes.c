 ...
 *  Otherwise, set to 1.
 *
 *  LOCAL CHANGE: we feed raw PPUMASK bit 5..7 into bits 6..8 without
 *  region-specific re-swapping, so on PAL hardware bit 6 = green (not
 *  red). Set this to 1 so PAL-CRT swaps internally to match our layout.
 *  See src/nes/palcrt/README.md.
 */
#define SWAP_RED_GREEN_EMPHASIS_BITS 1
...
pal_modulate(struct PAL_CRT *v, struct PAL_SETTINGS *s)
{
    ...
    for (x = 0; x < 4; x++) {
        for (n = 0; n < 6; n++) {
            v->ccf[n][x] = iccf[(n + 3) % 6][x] * 128; /* may be < 0 */
        }
    }
    v->cc_period = 6;
}
