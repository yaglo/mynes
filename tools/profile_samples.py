...
def main():
    ...
    for row in root.iter("row"):
        ...
        cores[resolve(row[3]).get("fmt", "unknown")] += weight
    ...
    if args.json:
        print(json.dumps({"samples": count, "cpu_ms": total / 1e6,
                          "groups_ms": {k: v / 1e6 for k, v in groups.items()},
                          "cores_ms": {k: v / 1e6 for k, v in cores.items()},
                          "functions_ms": {k: v / 1e6 for k, v in functions.items()}}))
        return
